...
anthropicRoutes.post(
  ...
  async (c) => {
    ...
    // Inject placeholder context into system prompt so LLM understands placeholders
    if (config.masking.inject_placeholder_context) {
      const mergedContext = mergeContexts(
        piiMaskingContext ?? createPlaceholderContext(),
      ...
      if (contextDesc) {
        if (typeof request.system === "string") {
          request.system = `${contextDesc}\n\n${request.system}`;
        } else if (Array.isArray(request.system)) {
          request.system = [{ type: "text", text: contextDesc }, ...request.system];
        } else {
          request.system = contextDesc;
        }
      }
