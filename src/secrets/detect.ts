...
import { createPatternDetectors } from "./patterns";
import type {
...
export function detectSecrets(
  ...
  // Get all detectors (built-in + custom from config)
  const detectors = createPatternDetectors(config.custom_patterns);
  ...
  for (const detector of detectors) {
    // Custom detectors (with dynamic patterns) always run if they exist
    const isCustomDetector = !detector.patterns.every((p) => enabledTypes.has(p));
    if (!isCustomDetector) {
